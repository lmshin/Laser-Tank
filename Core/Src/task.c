...
TaskHandle_t xHandleMain, xHandleTestTask;
...
//					&xHandleTask );		/* We are not using the task handle. */
	...
	//Gimbal 제어 테스크 생성
	xTaskCreate(
	    (TaskFunction_t)GimbalControlTask,  // Task 함수 포인터
	    "GimbalTask",                        // 디버깅용 이름
	    configMINIMAL_STACK_SIZE * 2,        // 스택 크기 (필요시 조정)
	    NULL,                                // Task 매개변수
	    GIMBAL_TASK_PRIO,                    // Task 우선순위
	    &xHandleGimbal                        // Task 핸들
	);

	//메세지를 보내기 위한 Tesk Task 생성
	xTaskCreate( (TaskFunction_t)TestTask, "TestTask", 256, NULL, RX_TASK_PRIO, &xHandleTestTask );
...
TaskHandle_t RemoteRxTaskHandle = NULL;
...
void vIR_Recv_Pin_IRQHandler( void )
{
    ...
    if( RemoteRxTaskHandle != NULL )
    {
        // RemoteRxTask에게 "새로운 펄스가 도착했다"고 알림
        // (ISR 내에서 사용 가능한 API)
        vTaskNotifyGiveFromISR( RemoteRxTaskHandle, &xHigherPriorityTaskWoken );
    }
...
void TestTask( void *pvParameters )
{
	const char *pcTaskName = "TestTask";

	Gimbal_Message_t xMessage;
	BaseType_t p;
	int i;
...
//	printf("\n-------  TestTask parameter passed from main --------\n");
...
//	printf("--------------------------------------------------\n");
	...
	xMessage.ucMessageID = Test_TASK_ID;
	...
	//p = xQueueSend(qid,&xMessage,pdMS_TO_TICKS (1000));

	for(i=1; i< 10000; i++) {
		xMessage.motorCommand = VERTICAL_DOWN;
		p = xQueueSendToFront(qid,&xMessage,portMAX_DELAY);
		if (p != pdPASS) printf("xQueueSendToBack error found\n");
		vTaskDelay (pdMS_TO_TICKS (1000));

		xMessage.motorCommand = VERTICAL_UP;
		p = xQueueSendToFront(qid,&xMessage,portMAX_DELAY);
		if (p != pdPASS) printf("xQueueSendToBack error found\n");
		vTaskDelay (pdMS_TO_TICKS (1000));

		xMessage.motorCommand = HORIZONTAL_LEFT;
		p = xQueueSendToFront(qid,&xMessage,portMAX_DELAY);
		if (p != pdPASS) printf("xQueueSendToBack error found\n");
		vTaskDelay (pdMS_TO_TICKS (1000));

		xMessage.motorCommand = HORIZONTAL_RIGHT;
		p = xQueueSendToFront(qid,&xMessage,portMAX_DELAY);
		if (p != pdPASS) printf("xQueueSendToBack error found\n");
		vTaskDelay (pdMS_TO_TICKS (1000));
...
void DriveControlTask( void *pvParameters )
{
    ...
    // 큐 생성
    DriveQueue = xQueueCreate(QUEUE_LENGTH, sizeof(Message_t));
    if (DriveQueue == NULL) {
        // 오류 처리
...
void LaserControlTask( void *pvParameters )
{
    ...
    // 큐 생성
    LaserQueue = xQueueCreate(QUEUE_LENGTH, sizeof(Message_t));
    if (LaserQueue == NULL) {
        // 오류 처리
